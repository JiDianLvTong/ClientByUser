...
public class DownLoadAndInstallBluetooth {
    ...
    public DownLoadAndInstallBluetooth(Activity activity, String versionUrl, String versionName, Handler returnHandler) {
...
//        request.setMimeType(mimeString);
        ...
        request.setDestinationInExternalPublicDir(Environment.DIRECTORY_DOWNLOADS, "hlhdapp.bin");
        ...
        if (!file_1.exists()) {
            ...
            if (file_1 != null) {
                ...
                if (Build.VERSION.SDK_INT >= 24) { //判读版本是否在7.0以上 //参数1 上下文, 参数2 Provider主机地址 和配置文件中保持一致 参数3 共享的文件
                    Uri apkUri = FileProvider.getUriForFile(activity, "com.android.jidian.client.fileprovider", file); //添加这一句表示对目标应用临时授权该Uri所代表的文件
