...
//            ExampleUtil.showToast(logs, context);
            if (tagAliasBean.action == ACTION_SET) {
                SharedPreferences sharedPreferencesAlias = context.getSharedPreferences("aliasInfo", Activity.MODE_PRIVATE);
                SharedPreferences.Editor editor = sharedPreferencesAlias.edit();
                editor.putBoolean("isAliasSet", true);
                editor.putString("alias", jPushMessage.getAlias());
                editor.apply();
            } else if (tagAliasBean.action == ACTION_DELETE) {
                SharedPreferences sharedPreferencesAlias = context.getSharedPreferences("aliasInfo", Activity.MODE_PRIVATE);
                SharedPreferences.Editor editor = sharedPreferencesAlias.edit();
                editor.clear();
                editor.apply();
